...
MEP sits at the intersection of industrial operations, AI agent architecture, and distributed systems. The terminology reflects that lineage â€” borrowed from oil rigs, recontextualized for stateless machines, and occasionally invented from scratch.
...
When an AI coding agent session ends on one machine and another begins on a different machine, someone has to carry the context across. That someone re-explains what happened, re-establishes decisions, re-describes work in progress. They aren't thinking. They aren't deciding. They're *relaying* â€” acting as a biological message bus between two stateless endpoints.

That's a meat puppet.

MEP exists to eliminate this role entirely. The human should be the *operator* â€” directing, deciding, steering â€” not the *transport layer*.

**Origin:** Named March 22, 2026 by Pierre Hulsebus on the occasion of his 62nd birthday. The term is deliberately provocative. If it makes you uncomfortable, good â€” that discomfort is the point. You shouldn't be doing this job.
...
Not a conversation log. Not raw memory. A curated shift-change briefing â€” what happened, what's pending, what to watch out for. Newest entry on top. Concise.

The baton is the atomic unit of MEP. Without it, there is no protocol â€” just two disconnected sessions.

**Format:** Markdown (reference implementation). Any human-readable structured text works. The requirement is *structure*, not format â€” because structure is what enables autonomous reasoning during merge conflicts.
...
Properties:
- **Structured format** â€” agreed upon across the organization, not free-form
- **Signed by the outgoing operator** â€” the signature is the authorization
- **Read by the incoming shift director** â€” who arrives asynchronously and may be unknown to the writer
- **Zero-trust model** â€” you don't trust what someone *tells* you; you trust what's *written and signed*
- **Secured at rest** â€” the signature is the security primitive
...
- Concise â€” this is a briefing, not a novel
...
A markdown file (typically `CLAUDE.md`) at the repo root that the AI agent loads automatically at session start. The protocol instructions â€” including "read the handoff file" â€” are embedded directly in this file.
...
Traditional protocols require someone to *invoke* them â€” run a script, start a service, execute a command. MEP requires nothing. The identity file loads automatically. The instructions are mandatory. The agent reads, follows, and executes without prompting.
...
When the human signals session end â€” via keyword (`/eol`, `p-out`, `ppp`) or natural language ("done", "heading out", "wrap up") â€” the agent executes the shutdown sequence:
...
**Cultural reference:** "End of Line" â€” Tron (1982). The MCP's final words. Also: the terminating character in legacy terminal protocols. Both meanings apply.
...
**The "Do Nothing" security advantage:** GitHub as transport satisfies enterprise security requirements â€” encryption at rest, 2FA, access control, audit log â€” with zero additional infrastructure or management overhead.
...
The hardest requirement in enterprise AI adoption isn't the model â€” it's security, auditability, and data governance. MEP meets all three by using Git/GitHub as the transport layer:
...
| Audit trail | Git commit history â€” timestamped, attributed, immutable |
...
A session is stateless â€” it begins from zero and ends completely. Nothing persists between sessions except what is written to the [baton](#baton) and committed to the [transport layer](#transport-layer).
...
In MEP, the operator is the decision-maker â€” not the relay. They steer, they direct, they approve. They do not re-explain context. They do not re-establish decisions. That's the [meat puppet's](#meat-puppet) job, and MEP eliminated it.
...
Not simultaneous â€” asynchronous by design. One session ends. Time passes. Another session begins on a different machine (or the same machine). The [baton](#baton) bridges the gap.
...
- Doesn't matter â€” the structured format carries the context regardless of identity
...
MEP was originally designed to carry context between sessions. The [First Autonomous CI Recovery milestone](spec/mep-protocol.md#milestone-first-autonomous-ci-recovery) (April 3, 2026) proved it can also close the *CI feedback loop* â€” diagnosing and fixing merge conflicts by reasoning about the baton's structure.
...
The conversation URL is the [baton](#baton) â€” different transport (HTTPS vs Git), same pattern.  The operator is no longer locked into one AI ecosystem to avoid the re-explanation cost.
...
The seed prompt is the manual fallback for when self-enforcement isn't possible.  It's the minimum viable meat puppet action â€” one paste instead of twenty answers.
...
Every handoff â€” machine-to-machine, LLM-to-LLM, disconnected session bootstrap â€” must minimize the number of human actions required.  If the human has to answer questions, re-explain context, or navigate setup wizards, the friction is too high.
...
Grok brainstorms don't float free â€” they enrich a specific skill.  ChatGPT research doesn't live in ChatGPT â€” it routes into the project it serves.  Claude is the hub because it has code execution, repo access, and durable memory.  The hub must be the LLM that can *act* on insights, not just discuss them.

Every cross-ecosystem conversation is a project contribution.  The project accumulates context from every LLM the operator uses.  Next time anyone opens that skill â€” on any machine, in any session â€” all the insights are already there.
...
Claude is the hub â€” it has repo access, code execution, durable memory, and the ability to act on insights.  Other LLMs (Grok, ChatGPT, Gemini) are spokes â€” they excel at specific tasks (brainstorming, research, voice work) but their outputs must route to the hub to become permanent and actionable.
...
When an operator shares a Grok, ChatGPT, or Claude conversation URL, the full conversation becomes a readable artifact â€” similar to the [baton](#baton) but authored by a different AI provider.  The receiving agent fetches it, parses the conversation structure, and extracts context.

Unlike handoff files (which the operator's agent writes), conversation artifacts are authored by a foreign LLM.  The receiving agent must interpret another provider's output â€” a form of cross-ecosystem literacy.
...
Project-scoped â€” each project gets its own Standing Standup.  Every agent reads it before working and appends when done.  No facilitator needed.  The meeting is always in progress.

Contains:
- **Scope** â€” what this project is
- **Artifacts** â€” what files exist and where
- **Pointers** â€” public URLs to read (specs, conversations, docs)
- **Party line** â€” agents working on this project
- **Standup log** â€” tag-in/tag-out entries (newest first)
...
The Standing Standup eliminates the "Unimind" problem â€” no universe-scoped document that tries to contain everything.  Each project's standup stays focused on that project's context.  Agents only see what they need.
...
Not a separate format â€” the same handoff schema with tag-in/tag-out headers.  Lives on Google Drive.  Claude, Grok, ChatGPT, Gemini all read it on session start and append entries on session end.  Static context header (who Pierre is, projects, crew, voice rules) plus rolling handoff entries.

The outbound baton is a projection of the repo â€” what peer agents need to know, sanitized of private details.
...
Extended header format: `## DATE â€” Agent | Platform | session-type` followed by `**Tag-in:** TIME | **Tag-out:** TIME`.  Inherited from industrial shift changes â€” the outgoing operator signs out, the incoming operator signs in.  In MEP, every agent tags in when they start reading the file and tags out when they append their entry.

Backward-compatible with v1 headers.  A v1 entry `## 2026-04-13 â€” Hot Rod (main)` is valid v2 with agent/platform inferred.
...
Grok and ChatGPT don't have Git access.  They can't clone skippy-brain.  But they can read a Google Drive file.  The outbound baton is a "poor man's repo" â€” enough context to make every agent productive without the full repo infrastructure.
...
```
Claude publishes â†’ Peer reads â†’ Peer contributes â†’ Pierre pastes URL â†’
Claude ingests â†’ Claude archives â†’ Claude routes to project â†’
Claude updates outbound baton â†’ Peer reads updated context â†’ ...
```
