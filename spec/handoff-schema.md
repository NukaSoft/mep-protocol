# MEP Handoff Schema â€” Formal Specification
...
date-header     ::= "## " ISO-DATE " â€” " machine-context
...
**Entries MUST be ordered newest-first.** The most recent entry appears at the top of the file. This is not a convention â€” it is a protocol requirement. The autonomous conflict resolution capability of MEP depends on this ordering being machine-parseable.

```
## 2026-04-03 â€” Hot Rod (main)        â† newest, read first
...

## 2026-04-02 â€” Hot Rod (main)        â† previous
...

## 2026-04-01 â€” Mac (Nagatha/worktree) â† oldest in window
...
```
...
### `### What happened`
**Required.** A concise record of actions taken this session. Bullet list. Not prose. Focus on deliverables and decisions â€” not process.

Good:
```
- Built `/iterate` skill â€” 7-stage loop, Run Sheet template, symlinked
- Resolved merge conflict in handoff.md â€” PR #12 rebased and pushed
```
...
```
- [ ] **[Pierre]** Post Reddit drafts â€” 5 posts ready
...
```
- `~/.leo/` still exists as backup â€” can delete once confirmed
- Google Tasks MCP auth broken â€” needs `node auth.js`
```
...
1. **Structured diff support** â€” The transport must allow the receiving agent to compare versions and identify which entry is newer. Git satisfies this natively.
2. **Conflict resolution** â€” The transport must support merging divergent versions without data loss. Git satisfies this via rebase/merge semantics.
3. **Encryption in transit** â€” Required for enterprise deployments. Git over SSH or HTTPS satisfies this.
4. **No mandatory locking** â€” The transport must not use file locking. SMB/UMB file shares fail this requirement due to non-deterministic lock behavior under concurrent AI agent access.

### Conformant Transports
| Transport | Diff | Conflict Resolution | Encryption | No Lock | Conformant |
|-----------|------|--------------------|---------|---------|----|
| Git (GitHub/GitLab) | âœ… | âœ… | âœ… | âœ… | **YES** |
| Git (self-hosted) | âœ… | âœ… | âœ… (SSH) | âœ… | **YES** |
| SFTP + manual merge | âœ… | Manual | âœ… | âœ… | Partial |
| SMB/UMB file share | âŒ | âŒ | Optional | âŒ | **NO** |
| iCloud Drive | âŒ | âŒ | âœ… | âŒ | **NO** |
| OneDrive | âŒ | âŒ | âœ… | âŒ | **NO** |
| Google Docs | âŒ | âŒ | âœ… | âŒ | **NO** |
...
> Given two divergent versions of a handoff file (branch A and branch B), the agent can resolve the merge correctly â€” placing the newer entry on top â€” without human instruction, using only the structure of the existing entries as a guide.
...
The shared surface file in multi-agent MEP is called a **Standing Standup** â€” a persistent standup meeting with history and pointers.  It is always **project-scoped**: each project gets its own standup file.  Agents only see context for the project they are working on.
...
This eliminates the "Unimind" anti-pattern â€” a universe-scoped document that tries to contain everything becomes unusable.  Project-scoped standups stay focused and actionable.
...
1. **Tag-in / Tag-out** â€” Which agent wrote this entry? When did they start and stop?
2. **Multi-agent append** â€” Any agent can add an entry.  Newest on top.  Same rules.
3. **Shared surface** â€” One file, not per-agent files.  Everyone sees everything.
4. **Platform identification** â€” Which LLM? Which session type?
...
date-header     ::= "## " ISO-DATE " â€” " agent-context
...
```markdown
## 2026-04-13 â€” ChatGPT | ChatGPT | research
**Tag-in:** 10:00 ET | **Tag-out:** 10:45 ET
```

```markdown
## 2026-04-13 â€” Pierre | Human | review
**Tag-in:** 08:00 ET | **Tag-out:** 08:15 ET
```

### Backward Compatibility

The v2 header is a superset of v1.  The v1 format `## DATE â€” machine-context` parses as v2 with agent-name = machine-context and platform/session-type omitted.  Existing handoff files remain conformant.
...
### Watch out for
- Pierre was fatigued by hour 3 â€” stubborn-when-tired pattern noted
- Voice biometric insight needs careful implementation â€” practical (pipes are easy to type) not romantic (typewriter history)
```
...
- If two agents write simultaneously (rare â€” Pierre usually works with one at a time), the later save wins but no content is lost because entries don't modify previous entries
...
- Machine-to-machine (Claude Hot Rod â†’ Claude Mac)
- LLM-to-LLM (Grok â†’ Claude â†’ ChatGPT)
