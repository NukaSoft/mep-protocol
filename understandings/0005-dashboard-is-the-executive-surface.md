...
Today we collapsed that loop. Pierre opens one URL on his iPad and everything Skippy has ever published is sitting right there â€” research reports as markdown with tap-to-read navigation, PDFs with inline iframe preview so he can peek at layout without a download, annotations that autosave per report, theme that persists across tabs and devices, back buttons that never leave a dead end. The cost of publishing one more report â€” or one more PDF â€” goes to zero.

But the real unlock isn't the UI. It's the mandate we captured alongside it: **teach the bot once.** The `dashboard` skill now carries a "First Thing: Read Your References" gate and a Fit-and-Finish checklist that blocks any dashboard page from being called "done" until every item passes. Four reference docs (`page-template`, `theme-system`, `navigation`, `annotations`) capture *how* we built this â€” not as tutorials, but as pattern law. Next time any Skippy session touches the dashboard, it inherits the checklist instead of reinventing it. Inconsistency is the bug. Consistency is the product.

The architectural insight: the dashboard is the surface where everything the system produces has to land, or it doesn't exist. Reports that live in `/daily/` are invisible. PDFs that live in `docs/` are invisible. Anything that doesn't make it onto a card on the dashboard might as well have never been written. A surface that's easy to publish to becomes the organizing principle for the whole agent fleet â€” because the agents now have somewhere to deliver that Pierre will actually look at.

This is the same shift that made Gmail beat desktop mail clients, and the same shift that made the App Store beat shareware CDs. Not "better rendering" â€” **eliminated coordination cost**. When the cost of adding one more thing goes to zero, the whole pipeline reorients around the surface.

Pierre called it the executive dashboard. He's right, but it's deeper than that. Every artificial person on the crew â€” Rita's content, Piper's bug digests, Bishop's health reports, Jo's Powered Wild numbers, Bluto's ASU status â€” they all have the same publishing destination now. A single cohesive product surface instead of a sprawl of ad-hoc files.

And because the pattern is captured in the skill instead of the code, the next surface â€” Bishop page retrofit, crew profiles, project dashboards, D365 market briefings â€” all inherit the work for free.
...
- **Proof:** The dashboard-polish-sprint report published itself as the first test of its own pattern. The PDF Library landed the same afternoon as a clean extension â€” add a section, add a manifest, add a helper script, done. Total elapsed: under an hour. First attempt. No pattern re-derivation.
- **Quote:** Pierre, on seeing the first report render â€” "This is the most amazing day of my life. Brought tears to my eyes."
- **Quote:** Pierre, on seeing the PDF Library land â€” "THis is the perfect executive dashboard."
- **Quote:** Pierre, one beat later â€” "This Model Kills Everything like Power BI as the 'Composable Front End.'"

## The Power BI Killshot

Power BI's value prop was always "one place to look at everything." It won because it beat the alternative (no dashboard). But the model underneath it is rigid â€” dataset â†’ semantic model â†’ report â†’ workspace â†’ license seat â†’ tenant. Every new thing you want to surface has to be modeled into the framework. Every custom visual is a plugin negotiation. Every cross-source join is a data engineering project. The tool is expensive not because the software is expensive but because the **coordination cost of adding one more thing** is enormous.
...
1. **I talk** â€” voice in, transcript via Plaud or direct instruction to Skippy.
2. **Hey put this on the Dashboard** â€” the command. Not "file a ticket," not "open a PR," not "schedule a meeting with the BI team." Just the instruction, spoken.
3. **You go do magic calculations** â€” agent computation. Skippy (or Rita, Piper, Jo, Bishop, Garrus, whoever owns the domain) does the research, the analysis, the rendering, the PDF generation, the whatever.
4. **Then you build** â€” publish via `publish-report.sh` or `publish-pdf.sh`. File on disk. Manifest updated. Done.
5. **I see it** â€” Pierre refreshes the iPad bookmark. It's there. Tap to read. Annotation box waiting.
...
That's the security model that lets the dashboard be an executive surface in a way Power BI never could be â€” because Power BI ships your data to Microsoft's cloud by design, and Hot Rod never does.

## Related

- `reports/view.html?r=dashboard-polish-sprint` â€” the self-documenting sprint report
- `reports/view.html?r=gastown-borrow-spec` â€” the Gas Town architectural review that sparked the session
- `skills/dashboard/SKILL.md` â€” source of truth
- `skills/dashboard/references/` â€” the four reference docs
- `docs/dashboard/hotrod-www/` â€” recoverable snapshots
- `scripts/publish-report.sh`, `scripts/publish-pdf.sh` â€” the publishing helpers
